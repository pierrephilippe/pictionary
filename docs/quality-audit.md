...
| Dessin simple | Canevas principal, menu d’outils repliable, mot et minuteur visibles, sélection du gagnant séparée. | Composants présents ; ergonomie tactile à tester. |
| Projection épurée | Pendant `drawing`, les informations de jeu sont masquées et seule la toile est rendue ; une barre technique temporaire apparaît après interaction. En cas de coupure, une alerte de reprise remplace explicitement l’image figée. | Branche de rendu et recette navigateur ; projection physique requise. |
